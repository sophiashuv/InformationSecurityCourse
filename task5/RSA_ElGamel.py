...
class RSACipher:
    ...
    def getCipherText(self, public_rsa):
        e, n = public_rsa
        res = [pow(ord(c), e, n) for c in self.massage]
        res_m = "".join(chr(b) for b in res)
        return res_m

    def getOriginalText(self, private_rsa):
        d, n = private_rsa
...
class ElGamelSign:
    ...
    def sign(self, prime_p, g_left, private_key):
        ...
        ll = modul_eq_one(k, prime_p - 1)
        s = ((hash_mes - private_key * rr) * ll) % (prime_p - 1)
        lines = [encrypted_msg, "|", str(rr), "|", str(s)]
        encoded = "".join(lines)
        return encoded

    def verify_and_decode(self, prime_p, g_left, public_key, private_rsa):
        ...
        if l == r:
            ...
            return r.getOriginalText(private_rsa)
        else:
...
if __name__ == "__main__":
    P, Q = RSACipher.generate_p_q(2 ** 4)
    public_rsa, private_rsa = RSACipher.generate_keys_rsa(P, Q)

    g = prime_root(P)
    privateKey, publicKey = ElGamelSign.generate_keys(P, g)

    text_file = "../text.txt"
    message = read_txt(text_file)

    r = RSACipher(message)
    encrypted_msg = r.getCipherText(public_rsa)
    ...
    decoded = e.verify_and_decode(P, g, publicKey, private_rsa)
    print(decoded)
